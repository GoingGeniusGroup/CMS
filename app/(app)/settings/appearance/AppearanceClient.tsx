...
export default function AppearanceClient({ initialData }: { initialData: AppearanceData }) {
  const [baseColor, setBaseColor] = useState(initialData.baseColor);
  ...
  function handleSave() {
    startTransition(async () => {
      const result = await saveAppearanceSettings({ baseColor, hoverColor, timezone });
  ...
  return (
    <Card className="lg:p-8">
      ...
      <div className="mt-8 grid grid-cols-1 gap-6 sm:grid-cols-2">
        <ColorField label="Website Base Color" value={baseColor} onChange={setBaseColor} />
        <ColorField label="Website Base Hover Color" value={hoverColor} onChange={setHoverColor} />
      </div>
