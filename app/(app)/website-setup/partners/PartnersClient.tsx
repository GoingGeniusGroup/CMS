...
export function PartnersClient({ initialPartners, initialBgColor, initialTextColor }: { initialPartners: string[]; initialBgColor?: string; initialTextColor?: string }) {
  const [partners, setPartners] = useState<string[]>(initialPartners);
  const [bgColor, setBgColor] = useState(initialBgColor || "#09090b");
  const [textColor, setTextColor] = useState(initialTextColor || "#a1a1aa");
  ...
  async function handleSave() {
    setIsSaving(true);
    setMessage(null);
    const result = await saveSetting("partners-logos", { partners, bgColor, textColor });
    setIsSaving(false);
    if (result.success) {
    ...
    <div className="space-y-5 sm:space-y-6">
      ...
      <Card className="p-6">
        ...
        <div className="mt-4 rounded-xl p-4" style={{ backgroundColor: bgColor }}>
          <p className="text-center text-sm font-bold" style={{ color: textColor }}>Preview: PARTNERS</p>
        </div>
      </Card>
      ...
      <div className="sticky bottom-4 z-10 flex items-center justify-end gap-2 rounded-2xl border border-zinc-200 bg-white px-6 py-3 shadow-sm">
        <button
          ...
          onClick={() => {
            ...
            setBgColor(initialBgColor || "#09090b");
            setTextColor(initialTextColor || "#a1a1aa");
            partnersRef.current = initialPartners;
          }}
          disabled={isSaving}
